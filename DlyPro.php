		...
		while ($empnors = $MYdb->Next_Record($active_emp))//loop runs for all Espected employees
		{
			...
			else
			{
				...
				AttendDate = '$dt' AND (AttendTime >= '12:00:00' AND AttendType = 'F2');"))==1))
				{
					...
					$actintime = new DateTime($MoInInfo['AttendTime'], $timezone);
					...
					$actouttime = new DateTime($EvOuInfo['AttendTime'], $timezone);
					...
					if ($MYdb->Row_Count($midoutsandins)== 2)			
					{
						...
						//check whether he has taken a short leave in the middle of the day (F3)
						{
							...
							//check whether he has taken a short leave in the middle of the day (F4)
							{
								...
								$SlvIntime=new DateTime($SlvInInfo['AttendTime'], $timezone);
				...
				else
				{
					...
					AttendDate = '$dt' AND (AttendTime >= '12:00:00' AND AttendType = 'F2');"))==1)))
					{
						...
						$testvaltime1=new DateTime($testvalinfo1['AttendTime'], $timezone);
						$testvaltime2=new DateTime($testvalinfo2['AttendTime'], $timezone);
						...
						{
							$actintime=new DateTime($testvaltime1->format('H:i:s'), $timezone);
							$actouttime=new DateTime($testvaltime2->format('H:i:s'), $timezone);
