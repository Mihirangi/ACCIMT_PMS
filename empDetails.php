...
<body onLoad="sessSet('empDetails.php')">
			<?php
				...
				if(isset($_POST['btnadd'])){
					if (empty($_POST["aaa"])){
						...
						if(mysql_num_rows($result1)==0)
						{
							...
							$query3 = "Insert emp_details_tbl(EmpNo,Title,Fullname,NICNum,DOB,Gender,Email,MobileNo,DateofAppoinment,DivisionCode,DesigCode,EmpTypeCode,CategoryCode) Values(	'".$empno1."',". "'" . $title . "', "."'".$fullname."',"."'".$nicnumber."',"."'".$dob."',"."'". $gender . "',". "'" . $email . "', "."'".$mobino."',"."'".$dateofappoinment."',"."'".$row[0]."',"."'".$row[1]."',"."'".$row[2]."',"."'".$row[3]."');";
				...
				if(isset($_POST['btnedit'])){ // Fetching variables of the form which travels in URL
					...
					$query = mysql_query("Update emp_details_tbl Set EmpNo='$empno2',Title='$title',Fullname='$fullname',NICNum='$nicnumber',DOB='$dob',Gender='$gender',Email='$email',MobileNo='$mobino',DateofAppoinment='$dateofappoinment',DivisionCode='$row[1]',DesigCode='$row[2]',EmpTypeCode='$row[3]',CategoryCode='$row[4]',RetirementDT='$retirementdate' where EmpNo='$row[0]'");
...
<?php
mysql_connect('localhost', 'root', 'pwditlinux');
			...
			<caption><h1>Employee Details </h1></caption>
					...
					<th valign="top">:</th>
							...
							<select name = "selectemployee" id = "selectemployee" onChange = "buttonfun(this.value)">
                                    <?php
										...
										mysql_select_db('attend_db');
										$values = mysql_query("SELECT EmpNo FROM emp_details_tbl ORDER BY EmpNo;");
				...
				<tr>
					...
					<th valign="top">:</th>
							...
							<td align="left"><select name = "txtdivicode" id = "txtdivicode">
                                    <?php
