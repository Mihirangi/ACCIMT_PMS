...
while($myrow = $db->Next_Record($myresult)) 
{
	...
	$bud_code= $myrow["bud_code"];
		...
		$db->Exe_Qry("UPDATE bud_req_hod SET appd_DG='Y', DG_appdtotal='$appAmount' WHERE  bud_year='$BudYerLst' and bud_code='$bud_code' and hod='$hod1'");
	...
	while ($roww = $db->Next_Record($pr_number))
	    {
		   ...
		   //echo $row[0]; $roww["user"]
	        ...
	        echo "<tr>
				<th><a target=".'"new"'." href=\"ddg_view.php?pr_no=".$roww["pr_no"]."\">".$roww["pr_no"]."</a></th>
