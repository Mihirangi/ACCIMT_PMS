...
else if (isset($_POST['btnAdd']))
{
	if (true){
		...
		$resss1=$db->Exe_Qry("SELECT COUNT(bud_code) FROM bud_req_user WHERE bud_year='2017' && bud_code='$bud_code';");
		$bud_codep=$db->Next_Record($resss1);
		if($bud_codep['COUNT(bud_code)']==0){
		$bud_coden=1;
		}
		else{
			$bud_coden=$bud_code.'/'.$div.'/'.$bud_year.'/'.$bud_codep['COUNT(bud_code)'];
