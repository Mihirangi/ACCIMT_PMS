  ...
  <table id="wrapped" align="center" cellspacing="3" cellpadding="5">
   <tr>
    ...
    if(isset($_POST['btnpros']))
	{
		...
		$act_emp_sql="SELECT e.EmpNo, c.InTime, c.OutTimeH, c.OutTimeM, c.AdjestTime, c.LateTime FROM emp_details_tbl e,category_tbl c WHERE e.RetirementDT NOT BETWEEN '$dummy_dt' AND '$dt' AND e.CategoryCode=c.CategoryCode;";
		//*
		//*
		//*
	...
	if ($MYdb->Row_Count($MYdb->Exe_Qry("SELECT DISTINCT EmpNo FROM attendance WHERE AttendDate = '$dt';"))!= 0)
	{
		...
		while ($empnors = $MYdb->Next_Record($active_emp))//loop runs for all Espected employees
		{
			...
			else
			{
				...
				if (($MYdb->Row_Count($MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND (AttendTime <= '12:30:00' AND AttendType = 'F1');"))==1) && ($MYdb->Row_Count($MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND (AttendTime >= '12:00:00' AND AttendType = 'F2');"))==1))
				{
					...
					$eveningOut=$MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND AttendTime >= '12:00:00' AND AttendType = 'F2';");
					...
					$midoutsandins=$MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND (AttendType = 'F4' OR AttendType = 'F3');");	
					...
					if ($MYdb->Row_Count($midoutsandins)== 2)			
					{
						$isf3sorf4s=true;
						$doProceed = true;
						$SlvOut=$MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND AttendType = 'F3';");
						...
						//check whether he has taken a short leave in the middle of the day (F3)
						{
							...
							$SlvIn=$MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND (AttendTime BETWEEN '".$SlvOuttime->format('H:i:s')."' AND '".$Slvlimittime->format('H:i:s')."') AND AttendType = 'F4';");
							...
							else
							{
								...
								$SlvIn=$MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND AttendType = 'F4';");
				...
				else
				{
					$testval1 = "";
					$testval2 = "";
					$testval1 = $MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND AttendTime <= '12:30:00' AND AttendType = 'F1';");
					$testval2 = $MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND AttendTime < '12:00:00' AND AttendType = 'F2';");
					
					if ((($MYdb->Row_Count($testval1)==1) && ($MYdb->Row_Count($testval2)==1)) || (($MYdb->Row_Count($MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND (AttendTime > '12:30:00' AND AttendType = 'F1');"))==1) && ($MYdb->Row_Count($MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND (AttendTime >= '12:00:00' AND AttendType = 'F2');"))==1)))
					{
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								$SlvIn2=$MYdb->Exe_Qry("SELECT AttendTime FROM attendance WHERE EmpNo='$empno' AND AttendDate = '$dt' AND AttendType = 'F4';");
			...
			$attType=1;
			echo "<br> ".$doProceed." intime ".$actintime->format('H:i:s')."<br><br>";
			//echo '<script> alert ("'. $isIssue .' - '. $isFDLeave .' - '. $isHDtaken .' - '. $isSLtaken .' - '. $isSLandHDtaken .' - '. $isAbsent .'"); <//script>';
			if ($isIssue)
			{
				$issueDesc = "";
			...
			if (($isIssue==false && $isAbsent==false && $daty!="2") || ($daty=="2" && $dychk==false && $isIssue==false))
			{
				echo "<br>".$empno."<br><br>";
	...
	?>
    <tr>
    <th>Active Number of Employees</th>
    <th><?php echo $aec;?></th>
    </tr>
    <tr>
    <th>Attended Number of Employees</th>
    <th><?php echo $pec;?></th>
    </tr>
    <tr>
    <th>Absent Number of Employees</th>
    <th><?php echo $abec;?></th>
    </tr>
    <tr>
    <th>Incomplete Attended Number of Employees</th>
    <th><?php echo $iec;?></th>
    </tr>
    <?php
	}
