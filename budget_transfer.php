...
{
	 $ress = $db->Exe_Qry("SELECT br_no FROM pc_bud_req_user");
...
if (isset($_POST['btnEdi']))
{
	...
	$db->Exe_Qry("UPDATE pc_bud_req_user SET equ_detail='$equ_detail',  purpose='$purpose',similar_equ='$similar_equ',reason='$reason', value='$value',target_month='$target_month',req_date='$req_date', bud_year='$bud_year', bud_code='$bud_code' WHERE pr_no='$PrCodeLst'");
...
else if (isset($_POST['btnAdd']))
{
	...
	$db->Exe_Qry("UPDATE pc_transfer_bud SET bud_code_frm='$touser',  amount='$value' WHERE bud_code_to='$budCode'");
...
{
	$db->Exe_Qry("DELETE FROM pc_bud_req_user WHERE pr_no='$PrCodeLst';");
...
else if (isset($_POST['PrCodeLst']) && ($_POST['PrCodeLst'] != $db->Row_Count($ressmytest)+1) && $_POST['PrCodeLst'] !="new" )
{
	...
	$PrCodeLst=$_POST['PrCodeLst'];
	//print $PrCodeLst;
	$res=$db->Exe_Qry("SELECT * FROM pc_bud_req_user WHERE pr_no='$PrCodeLst'");
  ...
  <table id="wrapped2" align="center" width="136%"  border="0" cellpadding="10" cellspacing="1">
   ...
   <h1>Budget Transfer</h1>
	  ...
	  <td><select name="budCode" id = "budCode"  onChange="document.form1.submit()">
	   <?php 
	   echo'<option value="new">Select Requested budget Codes</option>';
	   $empno = $_SESSION['proclogin_un'];
	   $ress = $db->Exe_Qry("select distinct(b.bud_code_to) from pc_bud_req_user a,transfer_bud b where a.status='t' && b.tra_user=$empno");
       ...
       <script>
	   document.getElementById('budCode').value="<?php echo $budCode;?>";
       </script>
	   ...
	   <th>:</th>
	  <td><select name="touser" id = "touser" onChange="document.form1.submit()">
	  
		  
		    <?php
          echo'<option value="new">Select transfer budget Codes</option>';
								
								
								$values =$db->Exe_Qry("select bud_code from pc_bud_req_user where user=$empno");
								echo '<option>--Select Transfer Budget code--</option>';
