  ...
  <table id="wrapped2" align="center" width="136%"  border="0" cellpadding="10" cellspacing="1">
   ...
   <tr>
	...
	$ress = $db->Exe_Qry("select * from designation_tbl WHERE DesigCode=(SELECT DesigCode FROM emp_details_tbl WHERE EmpNo='$empno')");
	   ...
	   $empno = $_SESSION['proclogin_un'];
	   $ress = $db->Exe_Qry("select * from budget_tbl ");
		while ($roww = $db->Next_Record($ress))
	    {
	        echo "<OPTION VALUE=".$roww["budg_code"].">".$roww["budg_code"]."</OPTION>";
		}
	   ?>
        ...
        <tr>
       <th align="left">Request Item</th>
       <th>:</th>
       <td><select name="reqitem" id = "reqitem"  onChange="document.form1.submit()" >
	   <?php 
	   echo'<option value="new">Please Select a Request Item</option>';
	   $empno = $_SESSION['proclogin_un'];
	   $ress = $db->Exe_Qry("select * from bud_req_user WHERE user='$empno' and bud_code='$budg_code'");
	   $qury1= "select * from bud_req_user WHERE user='$empno' and bud_code='$budg_code'";
	   print $qury1;
		while ($roww = $db->Next_Record($ress))
	    {
	        echo "<OPTION VALUE=".$roww["pr_no"].">".$roww["equ_detail"]."</OPTION>";
		}
	   ?>
      
       </select>
        <script>
	   document.getElementById('reqitem').value="<?php echo $reqitem;?>";
       </script></td>
      </tr>
