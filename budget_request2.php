...
if (isset($_POST['btnEdi']))
{
	...
	$db->Exe_Qry("UPDATE pc_bud_req_user SET  equ_detail='$equ_detail',  purpose='$purpose',similar_equ='$similar_equ',reason='$reason', value='$value',target_month='$target_month',req_date='$req_date', bud_year='$bud_year', bud_code='$bud_code' WHERE br_no='$PrCodeLst'");
...
else if (isset($_POST['btnAdd']))
{
	...
	$db->Exe_Qry("INSERT INTO transfer_bud (req_user,bud_code_to,amount,req_date) 
	VALUES ('".$_SESSION['proclogin_un']."','$bud_coden','$amount','$req_date');");
        
        	
	$db->Exe_Qry("INSERT INTO pc_bud_req_user(br_no,user,value,req_date,bud_year,bud_code,sub_bud_code,status) 
	VALUES ($PrCodeLst,'".$_SESSION['proclogin_un']."',$amount,'$req_date',$bud_year,'$bud_coden','$bud_code','t');");
        
       // $db->Exe_Qry("UPDATE bud_req_user set status='t' where br_no=$PrCodeLst");
...
{
	$db->Exe_Qry("DELETE FROM pc_bud_req_user WHERE br_no='$PrCodeLst';");
...
else if (isset($_POST['PrCodeLst']) && ($_POST['PrCodeLst'] != $db->Row_Count($ressmytest)+1) && $_POST['PrCodeLst'] !="new" )
{
	...
	$PrCodeLst=$_POST['PrCodeLst'];
	//print $PrCodeLst;
	$res=$db->Exe_Qry("SELECT * FROM pc_bud_req_user WHERE br_no='$PrCodeLst'");
...
<div id="wrapp">
 <form id="form1" name="form1" method="post" action="" onSubmit="" >
  <table id="wrapped2" align="center" width="136%"  border="0" cellpadding="10" cellspacing="1">
   <caption>
   <h1>User Budget Request for a  Transfer</h1>
   </caption>
  <tr>
	   ...
	   $ress = $db->Exe_Qry("SELECT br_no FROM pc_bud_req_user");
		...
		$ress1 = $db->Exe_Qry("SELECT br_no FROM pc_bud_req_user Where user='$_SESSION[proclogin_un]'");
	...
	echo '	<option value="'.(($db->Row_Count($ress))+1).'">New</option>';
	    ...
	    {
	        echo "<OPTION VALUE=".$roww1["br_no"].">".$roww1["br_no"]."</OPTION>";
		}
		echo'</select>';
