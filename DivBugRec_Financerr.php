...
$mysql="SELECT * FROM emp_details_tbl e, bud_req_user b,division_tbl d WHERE b.user=e.EmpNo and e.DivisionCode=d.DivisionCode and e.DivisionCode='$divitionlist' and b.recommend='y' order by b.bud_code ";
}
if( $flag1=='a'){
	
$mysql="SELECT * FROM bud_req_user WHERE  bud_year='$BudYerLst' order by br_no";
...
//print '<br>';	
	...
	$db->Exe_Qry("UPDATE bud_req_user SET  value_fin='$appAmount' ,balance='$appAmount' WHERE  bud_year='$BudYerLst' and bud_code='$bud_code' and user='$user'");	
...
if( $flag1=='i'){
	$sql="SELECT * FROM emp_details_tbl e, bud_req_user b,division_tbl d WHERE b.user=e.EmpNo and e.DivisionCode=d.DivisionCode and e.DivisionCode='$divitionlist' and b.recommend='y' order by b.bud_code";
//$sql="SELECT * FROM bud_req_user WHERE  bud_year='$BudYerLst' ";
}
else if ($flag1=='a')
{	
//print $BudYerLst;
$sql="SELECT * FROM bud_req_user WHERE  bud_year='$BudYerLst' order by br_no";
//print $sql;
}
...
while($row = $db->Next_Record($result)) {
	...
	echo '<td><input type="text" name="test'."$i".'" value="'.$row['value_fin'].'" > </td>';  
	echo "</tr>";
	$i++;
}
